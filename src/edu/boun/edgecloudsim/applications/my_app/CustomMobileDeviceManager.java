...
//		System.out.println("task.getAssociatedDatacenterId(): " + task.getAssociatedDatacenterId());
		...
		if (task.getAssociatedDatacenterId() == SimSettings.CLOUD_DATACENTER_ID) {
			...
			// Initialization
			// WLAN delay
			double delay = networkModel.getDownloadDelay(SimSettings.GENERIC_EDGE_DEVICE_ID, task.getMobileDeviceId(), task);;
			...
			// Get current edge host
			EdgeHost host = (EdgeHost)(SimManager
					.getInstance()
					.getEdgeServerManager()
					.getDatacenterList()
					.get(task.getAssociatedHostId())
					.getHostList().get(0));


			// From one edge device to a neighbor edge device
			...
			// Whether the neighbor edge device is chosen or not, the system will do the following
			if (delay > 0) {
				Location currentLocation = SimManager.getInstance().getMobilityModel().getLocation(
						task.getMobileDeviceId(), CloudSim.clock() + delay);
				if (task.getSubmittedLocation().getServingWlanId() == currentLocation.getServingWlanId()) {
	...
	protected void processOtherEvent(SimEvent ev) {
		...
		switch (ev.getTag()) {
			...
			{
				((CustomNetworkModel)networkModel).updateMM1QueeuModel();
